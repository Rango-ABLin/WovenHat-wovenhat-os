...
pub fn mount_ata_root() -> MountStatus {
    ata::with_primary_master(|disk| {
        let volume = match fat32::mount(disk) {
            Ok(volume) => volume,
        ...
        let count = match fat32::list_root(disk, volume, &mut entries) {
            Ok(count) => count,
            Err(_) => return MountStatus::Failed,
        };
        let mut mounted = 0;
        for entry in entries[..count].iter().flatten().copied() {
            if entry.attributes & DIRECTORY_ATTRIBUTE != 0
                || entry.size as usize > vfs::NODE_CAPACITY
            {
                continue;
            }
            ...
            let length = match fat32::read_file(disk, volume, entry, &mut data) {
                Ok(length) => length,
                Err(_) => return MountStatus::Failed,
            };
